    ...
    .login-container
        ...
        ul
            ...
            li
                ...
                span * 
                span 
                    input(type='text',placeholder='请输入提现金额(单位分)', v-model="loginForm.amount")
            li
                span 
                    svg-icon(icon-class="bet-record")
                span 
                    //- 订单号预留32位字符
                    input(type='text',placeholder='请输入代理商提现订单号', v-model="loginForm.agentTradeNo")
            li
                span 
                    svg-icon(icon-class="bet-record")
                span 
                    //- input(type='text',placeholder='格式：2019-6-2 22:00:56', v-model="loginForm.agentTradeTime")
                    el-date-picker(v-model="value", type="datetime", placeholder="请输入代理商提现订单的创建时间",  :default-time="defaultTime", size="mini")
            li
                button(type="primary",@click="handleLogin('withdrawal')") 确定
</template>
        ...
        handleLogin() {
            ...
            loginByToken(data).then(response => {
                ...
                if(res.success == 1){
                    ...
                    this.$store.dispatch('getAlertState', 'withdrawal') // alert提现状态
                } else {
